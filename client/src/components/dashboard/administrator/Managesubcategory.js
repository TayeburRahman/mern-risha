...
function ManageSubCategory() {
    ...
    useEffect(() => {
        axios.get(` https://server1.rishati.com/api/v1/category/signal/${locationID.id}`)
            .then((res) => {
    ...
    const handleDelete = (tittle) => {  
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(` https://server1.rishati.com/api/v1/category/subcategory/delete/${locationID?.id}/${tittle}`)
                .then((response) => {
                    swalWithBootstrapButtons.fire(
                        'Deleted!',
